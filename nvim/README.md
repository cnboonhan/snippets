...
`:checkhealth prereq` and `:PrereqInstall`, and installs only what is missing
from `PATH`. Homebrew itself is the one thing it will not install for you.
...
| `~/.zshrc` / `~/.profile` + `~/.bashrc` | both | `export CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1` — the alternate screen has no scrollback | `setup.sh` |
| `~/.ssh/config` | client | `Host X` → `SetEnv COLORTERM=truecolor` — without it nvim's `DCS +q` probe prints as junk | `ssh-client` |
| `/etc/ssh/sshd_config.d/99-colorterm.conf` | server | `AcceptEnv COLORTERM`, then `sudo sshd -t && sudo systemctl reload ssh` | `ssh-server` |
...
| `:PrereqInstall` / `:checkhealth prereq` | install missing tools / report them |
